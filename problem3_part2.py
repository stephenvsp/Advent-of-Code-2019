...
# given a (x ,y) coordinate and a direction, return the new coordinate
...
# return a list of every new coordinate the wire passes over in a given instruction


def parseInstruction(pos, instruction):
    direction = instruction[:1]
    numMoves = int(instruction[1:])
...
# calculate positions of wire one
wireOnePositions = [(0, 0)]

for instruction in wireOne:
    wireOnePositions.extend(parseInstruction(
        wireOnePositions[-1], instruction))

# calculate positions of wire two
wireTwoPositions = [(0, 0)]

for instruction in wireTwo:
    wireTwoPositions.extend(parseInstruction(
        wireTwoPositions[-1], instruction))

# calculate where the two wires intersect
intersections = list(set(wireOnePositions) & set(wireTwoPositions))
# Remove (0, 0)
...
for intersection in intersections:
    distanceOnWire = wireOnePositions.index(
        intersection) + wireTwoPositions.index(intersection)

    if (distanceOnWire < minDistance):
        minDistance = distanceOnWire
